...
`artifacts/performance/local-browser-smoke.json` 的最终完整 matrix Docs navigation 记录
CLS 0.000181、TBT 0、long task 0、LCP 532 ms 和约 44 ms
interaction-to-next-paint approximation。该 smoke 会把 Inter 字体请求额外延迟 300 ms，
因此能在每次运行中确定性覆盖首次访问的 fallback-font -> web-font 切换；同时保存每个
layout-shift 的时间、分值和 source rect，并按 Web Vitals 的 1 秒 gap / 5 秒 session
window 计算 CLS，而不是把全部 shift 无界相加。

PR #13 首轮 CI 在冷字体条件下发现 Docs `PageHeader` 的 CTA 会因字体 intrinsic width
从下一行跳回标题右侧，CLS 为 0.597358。共享 `PageHeader` 的 title/action row 现使用
明确的两列 grid，并在 980 px 以下切换为单列；测试还断言最终 computed `display` 为
`grid`，防止通用 row class 的 cascade 再次覆盖该约束。固定 300 ms 字体延迟后连续 3/3
复跑通过，且阈值仍为 0.1。所有优化都同时有行为回归测试，不能只凭 bundle 变小判定
成功。
