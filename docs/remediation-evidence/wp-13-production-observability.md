...
日期：2026-07-13
状态：**结构化事件与本地回归已就绪；Fugue Web 尚未发布，生产采样、观察窗口和回滚负责人未关闭。**
...
| `fugue_web_auth_rate_limit` | `policy`、`outcome=limited`；不记录 IP、email、bucket key 或 secret | 按 email/password/OAuth/finalize policy 统计 429 与异常命中率 | rate-limit unit/integration 与 429 产品场景 |
| `fugue_web_auth_email` | `flow`、`stage=delivery`、`outcome=failed`、受限 `category` | 统计 email-link / password-signup 邮件 provider 失败 | `auth-telemetry.test.ts` 与 Auth integration |
...
| `fugue_web_client_telemetry` | `kind`、route group；Web Vital 使用 `name/rating/value`，client error 使用受限 `source` | 按公开页/Auth/Console 路由组观察 Web Vitals 与客户端异常 | `client-telemetry.test.ts`、same-origin/body-limit tests |
