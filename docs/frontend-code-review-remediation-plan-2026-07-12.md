...
- [ ] PR 描述关联具体工作包、风险 ID、受影响路由和验收条件。
- [ ] PR 说明数据迁移、部署先后、兼容窗口和回滚方式。
...
- [ ] 性能 PR 包含 before/after 数据；视觉 PR 包含 desktop/mobile 证据。
...
- [ ] 为每个 commit/PR 标注对应工作包和风险 ID，并在 PR 描述提供逐提交 rollback map。
...
- [ ] 每个 Critical/High 风险都映射到至少一个实现 PR 和一个自动化测试。
...
- [ ] 将 Critical/High 对应测试设为 required checks。
...
- [ ] 契约、handler、前端和 contract CI 全链路通过。（本地 contract 门禁已通过；待 push 后确认远端 CI。）
...
- [ ] 项目所有者确认许可证策略；未确认前不得发布包含上游 AGPL source 的构建。
...
- [ ] 同步 PR 附 upstream changelog、许可证结论、受影响 consumers、截图和 bundle diff。
...
- [ ] COSS source 的 license/provenance/SBOM 已确认，发布产物不包含未批准 AGPL 或来源不明代码/字体/品牌资产。
- [x] 空库首用户不再自动获得 admin。
...
- [ ] Unit/integration/E2E/axe/style/audit/build CI 全绿。

### Gate C：产品质量

- [x] COSS 完整架构已生效，不只是页面风格相似。
- [x] 通用 UI 只有 `@fugue/ui` 一套 source/package 流，应用层没有第二套 primitive。
- [ ] License/provenance/SBOM Gate 已由项目 owner 确认。
...
| WP-05 | Codex | `12d0b16`、`c5e7500`；PR 待创建 | frozen install、security audit 0 advisory、lock review、versions gate 全绿 | lockfile source/install-script review 已归档 | 待发布 | Toolchain and local gates committed |
| WP-06 | Codex | `fugue@c7643ee/9cb0269/5246335`；Web `b892f09`、`c5e7500` | `fugue/main@710f7d2` snapshot hash parity + contract matrix 全绿 | 特殊协议 inventory 已签字式归档 | 后端 2026-07-12 | Backend authority released; Web remote CI pending |
| WP-07 | Codex | `b892f09`；PR 待创建 | server guard、Auth form、Env identity、authenticated E2E 全绿 | 产品行为自动化通过 | 待发布 | Local implementation committed |
| WP-08 | Codex | `b892f09`、`c5e7500`；PR 待创建 | build 同轮 bundle gate、19 routes、性能 artifacts 全绿 | 待发布后真实流量观测 | 待发布 | Local performance gate committed |
| WP-09A | Codex | `48367a0`；PR 待创建 | registry/build/sync/theme/provenance/license/SBOM gates 全绿 | **待 owner 明确许可证批准** | 待发布 | Architecture committed; owner gate pending |
| WP-09B | Codex | `b892f09`；PR 待创建 | consumer/source contracts、bundle、E2E 全绿 | Console desktop/mobile 缺陷已修复，截图重生成并人工复核通过 | 待发布 | Runtime migration and local visual acceptance committed |
