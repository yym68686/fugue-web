...
- [x] Push and deployment verification: pushed `0707d0a` to `main`, triggered Fugue build, confirmed `web.fugue.pro` deployed `git-0707d0a6a7fe` with 2/2 replicas ready, and repeated browser comparison across `/app`, `/app/apps`, `/app/api-keys`, `/app/billing`, `/app/cluster`, `/app/cluster-nodes`, `/app/settings/profile`, `/app/users`, and `/new/repository` until no unintended old component styling remained.
