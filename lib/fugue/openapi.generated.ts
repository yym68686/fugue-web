...
export interface paths {
  ...
  "/v1/admin/platform-config/routes/project": {
     ...
     * @description Read-only migration draft from one consistent business snapshot. PostgreSQL uses a read-only repeatable-read transaction; file storage reads under one lock. Returns the business snapshot revision/time and explicit migration issues. Runtime observations retain their own evidence timestamps and are not part of the database transaction. Desired release references and weights are projected into policy; release addresses and readiness retain their original observation timestamps in runtime_snapshot.releases. Application DNS placement capture probes each configured public candidate address with TLS hostname verification and the Edge route-proof protocol, checks every hostname path against the fixed compiled route, and binds the original inventory heartbeat and signed bundle expiry. Failed, stale or mismatched evidence stays an explicit issue; collection never extends a lease or authorizes promotion. migration_ready remains false until all serving inputs, policy execution, DNS/TLS readiness and output equivalence are verified. Writes no serving state.
     */
