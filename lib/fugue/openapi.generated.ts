...
export interface paths {
  ...
  "/v1/edge/route-intents": {
    /**
     * Edge Route Intents
     * @description Returns Core's inventory-independent desired-route projection. Only the edge-control component identity with global edge_route_intent capability is accepted; tenant and platform-admin API keys are not accepted. Returns only the newest applicable published gray/full TrafficReleaseSet projection for the required edge_group_id. Gray applies only to its signed cohort; a newer full supersedes an older gray lane. The selected release must have immutable expected topology for the group and verified parent/child integrity and lineage. Missing, invalid or unprepared releases return 503 so Edge Control retains its current serving artifact. This serving endpoint never reads mutable business tables or standalone route LKGs to synthesize configuration. Shadow releases never select serving content. Group projections retain traffic_release provenance; signatures include cache policies. Cache references resolve case-insensitively to the declared policy ID; disabled cache policies are not materialized as active bindings.
     */
  ...
  "/v1/database-migrations": {
    /** Create a generic managed database storage migration */
    post: operations["createDatabaseMigration"];
  };
  "/v1/database-migrations/{id}": {
    /** Get a database storage migration */
    get: operations["getDatabaseMigration"];
  };
  "/v1/operations": {
    /** List Operations */
...
export interface components {
  schemas: {
    ...
    DatabaseMigrationRequest: {
      ...
      kind?: string;
      /** @default control-plane-postgres */
      resource_id?: string;
      namespace?: string;
      cluster_name?: string;
      ...
      storage_size?: string;
      /** @default 1 */
      temporary_replica_count?: number;
    };
...
export interface operations {
    ...
    responses: {
      ...
      200: {
        headers: {
          ...
          /** @description Serving source for the route projection; always traffic-release. */
  ...
  createDatabaseMigration: {
    ...
    responses: {
      ...
      202: {
        content: {
          "application/json": components["schemas"]["DatabaseMigrationResponse"];
        };
      };
      default: components["responses"]["ErrorResponse"];
    };
