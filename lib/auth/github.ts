...
export function createGitHubAuthorizationUrl(state: string, codeChallenge: string) {
...
async function fetchGitHubPrimaryEmail(accessToken: string) {
  ...
  const fallback = payload.find((entry) => entry.verified && entry.email?.trim());
...
export async function fetchGitHubUser(accessToken: string) {
  ...
  const id =
    typeof payload.id === "number" && Number.isFinite(payload.id)
      ? String(payload.id)
      : "";
  const email = payload.email?.trim() || (await fetchGitHubPrimaryEmail(accessToken));
