...
export const GITHUB_APP_INSTALL_STATE_COOKIE_NAME =
  "fg_github_app_install_state";
...
async function fetchGitHubInstallationRepositories(
  ...
  let nextUrl: string | null = `https://api.github.com/user/installations/${encodeURIComponent(installationId)}/repositories?per_page=100`;
...
export function buildGitHubAppInstallStartHref(
  githubRepo: string,
  returnTo: string,
) {
...
export async function upsertGitHubAppInstallation(input: {
  ...
  return readGitHubAppInstallationFromRow(result.rows[0]);
}

async function readGitHubAppInstallationFromGitHub(
  token: string,
  githubRepo: string,
) {
  ...
  for (const installation of installationEntries) {
    ...
    const matched = repositories.some((repository) => {
      ...
      return fullName ? normalizeGitHubRepositoryName(fullName) === normalizedRepo : false;
    });
