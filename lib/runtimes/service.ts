...
import type { RuntimeShareGrantView, RuntimeSharingView } from "@/lib/runtimes/types";
...
function readTargetWorkspaceLabel(workspace: WorkspaceSnapshot | null, tenantId: string) {
  if (workspace?.email) {
...
async function buildRuntimeSharingView(
  ...
  const sharing = await getFugueRuntimeSharing(workspace.adminKeySecret, runtimeId);
  ...
  const ownerWorkspace =
    sharing.runtime.tenantId
      ? await getWorkspaceSnapshotByTenantId(sharing.runtime.tenantId)
      : null;
  const grants = await Promise.all(
    sharing.grants.map(async (grant) => {
      const targetWorkspace = await getWorkspaceSnapshotByTenantId(grant.tenantId);
...
async function resolveShareTargetWorkspace(
  ...
  if (existingUser) {
    throw new Error("409 This user needs to finish workspace setup before you can share a server.");
  }

  throw new Error("404 No Fugue user with this email was found.");
}

export async function readRuntimeSharingForEmail(email: string, runtimeId: string) {
...
export async function grantRuntimeShareForEmail(
  ...
  const workspace = await requireWorkspaceAccess(email);
  const targetWorkspace = await resolveShareTargetWorkspace(email, workspace, targetEmail);
...
export async function revokeRuntimeShareForEmail(
  ...
  await revokeFugueRuntimeAccess(workspace.adminKeySecret, runtimeId, tenantId.trim());
