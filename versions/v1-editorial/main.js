...
function initStageFocus() {
  const stage = document.querySelector(".orchestration-stage");
  const focusTriggers = Array.from(document.querySelectorAll("[data-focus]"));
...
function initStoryProgress() {
  ...
  if (!storyShell || steps.length === 0) {
    return;
  }

  const defaultProgress = "58%";

  const setActiveStep = (step) => {
    const progress = step.dataset.progress || defaultProgress;
    storyShell.style.setProperty("--story-progress", progress);
  ...
  const reset = () => {
    storyShell.style.setProperty("--story-progress", defaultProgress);
  ...
  steps.forEach((step) => {
    ...
    step.addEventListener("mouseleave", reset);
    step.addEventListener("blur", reset);
  });
}
