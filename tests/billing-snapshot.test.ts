...
test("billing snapshot accounts for every requested tenant", async (t) => {
  ...
  let payload: unknown = { billings: [{ tenant_id: "a" }], missing_tenant_ids: ["b"] };
  ...
  for (const invalid of [
    ...
    { billings: [{ tenant_id: "a" }], missing_tenant_ids: ["a", "b"] },
    { billings: [{ tenant_id: "foreign" }], missing_tenant_ids: ["b"] },
