...
export function readManifestBindingTargets(service: DeployInspectionManifestService) {
  return uniqueStrings(service.bindingTargets ?? []);
}

export function describeManifestServiceRole(service: DeployInspectionManifestService) {
  return humanizeDeployValue(service.serviceType || service.kind);
}

export function describeManifestBuild(service: DeployInspectionManifestService) {
  const parts = [
...
export function summarizeInspectManifest(manifest?: DeployInspectionManifest | null) {
