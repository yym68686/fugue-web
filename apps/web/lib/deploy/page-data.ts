...
import type { FugueGitHubTemplateInspection, FugueProject } from "@/lib/fugue/api";
import {
  getFugueProjects,
  getFugueRuntimes,
...
import { getWorkspaceAccessByEmail, type WorkspaceAccess } from "@/lib/workspace/store";
...
function readErrorMessage(error: unknown) {
  return sanitizePublicErrorMessage(error, readPublicErrorStatus(error));
}

function isUnauthorizedFugueError(error: unknown) {
  return readPublicErrorStatus(error) === 401;
}

async function loadInspection(search: DeploySearchState, session: SessionUser | null) {
    ...
    return {
      ...
      runtimeTargets:
        runtimesResult.status === "fulfilled"
          ? buildDeployRuntimeTargets(runtimesResult.value, workspace.tenantId, locale)
          : [],
      workspace,
      workspaceError: null,
    };
