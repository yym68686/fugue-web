...
function readFallbackWord(fallbackSeeds: readonly (string | null | undefined)[]) {
  if (!DEPLOY_APP_FALLBACK_WORDS.length) {
    return "app";
  }

  const normalizedSeeds = fallbackSeeds
    .flatMap((seed) => (typeof seed === "string" && seed.trim() ? [seed.trim()] : []))
    .join("\u0000");

  if (!normalizedSeeds) {
    const randomIndex = Math.floor(Math.random() * DEPLOY_APP_FALLBACK_WORDS.length);
...
export function normalizeDeployAppNameCandidate(value?: string | null) {
...
export function resolveDeployAppName(
  ...
  return (
    normalizeDeployAppNameCandidate(
      readFallbackWord(options?.fallbackSeeds ?? candidates),
    ) || "app"
  );
}

export function readRepositoryAppNameCandidate(repositoryUrl?: string | null) {
...
export function readImageAppNameCandidate(imageRef?: string | null) {
