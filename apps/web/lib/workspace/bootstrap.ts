...
import { readPublicErrorStatus } from "@/lib/security/public-error.mjs";
import {
  ensureAppUser,
...
function buildTenantName(email: string, name?: string) {
  const preferred =
    sanitizeDisplayName(name ?? "") || email.split("@")[0] || "workspace";
...
function resolveWorkspaceFirstApp(apps: FugueApp[], tenantId: string) {
...
async function backfillWorkspaceResourcePointers(workspace: WorkspaceAccess) {
  ...
  try {
    ...
    console.warn("Workspace resource pointer backfill failed.", {
      ...
      subject: buildTenantSuffix(workspace.email),
...
function findWorkspaceTenantForSession(tenants: FugueTenant[], session: SessionUser) {
  ...
  return (
    [...tenants]
      .filter(
        (tenant) =>
          tenant.name.endsWith(suffixOnName) || tenant.slug?.endsWith(suffixOnSlug),
      )
      .sort(
        (left, right) =>
          parseTimestamp(left.createdAt ?? left.updatedAt) -
...
async function reconcileWorkspaceAdminKey(
  ...
  const storedKey =
    workspace && tenantKeys.find((key) => key.id === workspace.adminKeyId);
  ...
  const needsMetadataRepair =
    apiKey.label.trim().toLowerCase() !== WORKSPACE_ADMIN_KEY_LABEL || needsScopeRepair;
  ...
  if (!(await hasUsableStoredAdminSecret(workspace, apiKey.id))) {
    console.warn(
      "Stored workspace admin key secret is no longer usable. Minting a fresh workspace admin key for this environment.",
      {
...
async function resolveWorkspaceTenant(bootstrapKey: string, session: SessionUser) {
  ...
  try {
    ...
    if (readPublicErrorStatus(error) !== 409) {
      throw error;
    }
...
async function ensureWorkspaceAccessInternal(session: SessionUser) {
  ...
  if (existing && !existing.adminKeySecret) {
    console.warn(
      "Workspace admin key secret could not be decrypted. Minting a fresh workspace admin key for this environment.",
      {
        subject: buildTenantSuffix(session.email),
