...
function buildRuntimePublicOfferView(
  offer: Awaited<ReturnType<typeof getFugueRuntimeSharing>>["runtime"]["publicOffer"],
) {
  if (!offer) {
    return null;
  }
  ...
  return {
    ...
    priceBook: {
      cpuMicroCentsPerMillicoreHour: offer.priceBook.cpuMicroCentsPerMillicoreHour,
...
export async function setRuntimePublicOfferForEmail(
  ...
  await setFugueRuntimePublicOffer(workspace.adminKeySecret, runtimeId, payload);
