...
async function attachNodeKeyUsageCounts(accessToken: string, keys: NodeKeyRecord[]) {
  if (!keys.length) {
    return keys;
  }
...
export async function getNodeKeyPageDataForWorkspace(
  ...
  if (didLiveSync && options?.includeUsageCounts !== false) {
    visibleKeys = await attachNodeKeyUsageCounts(workspace.adminKeySecret, visibleKeys);
  }
