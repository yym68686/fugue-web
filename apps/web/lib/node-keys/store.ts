...
function normalizeNodeKeyStatus(
  ...
  if (typeof status === "string" && status.trim().toLowerCase() === "revoked") {
    return "revoked";
  }
...
async function upsertVisibleNodeKey(client: PoolClient, input: SaveNodeKeyInput) {
  ...
  const status = normalizeNodeKeyStatus(input.nodeKey.status, input.nodeKey.revokedAt);
  const revokedAt =
    status === "revoked" ? (readTimestamp(input.nodeKey.revokedAt) ?? updatedAt) : null;
...
export async function getNodeKeyRecordById(email: string, fugueNodeKeyId: string) {
  const result = await withDbSchemaRetry(() =>
    queryDb<NodeKeyRow>(
      `
        SELECT
    ...
    await queryDb(
      `
        ...
        WHERE user_email = $1
          ...
          AND secret_sealed = $4
      `,
      [normalizeEmail(email), fugueNodeKeyId, resealed, secretSealed],
    );
  }
  return value;
}
