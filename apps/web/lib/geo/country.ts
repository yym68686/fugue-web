...
const COUNTRY_LABEL_OVERRIDES: Partial<
  Record<CountryCode, Partial<Record<Locale, string>>>
> = {
  HK: {
    en: "Hong Kong",
    "zh-CN": "中国香港",
...
const COUNTRY_NAME_TO_CODE = (() => {
  ...
  for (const code of COUNTRY_CODES) {
    const displayNames = [
      readRegionDisplayName("en", code),
      readCountryDisplayLabel(code, "en"),
    ];
...
export function readCountryCode(value?: string | null): CountryCode | null {
  ...
  return (
    (COUNTRY_NAME_TO_CODE.get(normalizeCountryKey(trimmed)) as
      | CountryCode
      | undefined) ?? null
  );
}
...
export function readCountryLocation(
  ...
  const locationLabelParts = [
    locationCountryLabel ?? normalizedRegion,
    normalizedZone,
  ...
  return {
    ...
    locationLabel: locationLabelParts.length
