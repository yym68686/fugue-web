...
import type { RuntimeOwnership, RuntimePublicOfferView } from "@/lib/runtimes/types";
...
function formatRelativeTime(locale: Locale, t: Translator, value?: string | null) {
...
function formatExactTime(locale: Locale, t: Translator, value?: string | null) {
...
function humanizeLabel(value: string | null | undefined, t: Translator) {
...
function formatCompactNumber(locale: Locale, value: number, digits = 1) {
...
function formatPercentLabel(locale: Locale, t: Translator, value?: number | null) {
...
function formatBytesLabel(locale: Locale, t: Translator, value?: number | null) {
...
function formatCPUCapacityLabel(locale: Locale, t: Translator, value?: number | null) {
  if (value === null || value === undefined || !Number.isFinite(value) || value < 0) {
    return t("No stats");
  }
...
function readOwnerTenantId(node: FugueClusterNode, runtime: FugueRuntime | null) {
...
function readResourceStatusKey(percent?: number | null, dangerSignal = false) {
  if (dangerSignal) {
    return "Pressure";
  }
...
function buildCPUResourceView(
  ...
  const requestedLabel = formatCPUCapacityLabel(locale, t, stats?.requestedMilliCores);
...
function buildMemoryResourceView(
  ...
  const capacityAmountLabel = isFiniteResourceValue(capacityBytes)
    ...
    : null;
  const schedulableFreeLabel = formatBytesLabel(locale, t, stats?.schedulableFreeBytes);
  const statusPercent = maxFinitePercent(percent, requestPercent);
...
function buildClusterConditionViews(
  ...
  return definitions.map((definition) => {
    ...
    return {
      ...
      statusLabel: t(readConditionStatusKey(definition.id, condition?.status)),
...
function buildClusterNodeViews(
  ...
  const views = visibleNodes.map((node) => {
    ...
    const isPublicRuntime = runtime?.accessMode?.trim().toLowerCase() === "public";
...
function buildOfflineServerViews(
  ...
  for (const node of nodes) {
    ...
    if (runtime?.id && !isWorkspaceOwnedOfflineRuntime(runtime, workspaceTenantId)) {
  ...
  const offlineOwnedRuntimes = runtimes
    ...
    .filter((runtime) => runtime.type?.trim().toLowerCase() !== "managed-shared")
  ...
  return {
    views: offlineOwnedRuntimes.map((runtime) => {
      ...
      const lastContactAt =
        ...
        runtime.createdAt ??
        null;
      const publicAccessEnabled = runtime.accessMode?.trim().toLowerCase() === "public";
