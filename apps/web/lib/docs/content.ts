...
const EN_CONTENT: DocsPageContent = {
  ...
  hero: {
    ...
    notes: [
      ...
      {
        index: "02",
        meta: "`persistent_storage_seed_files` only works when topology detection stays fully automatic.",
        title: "Seed-file overrides are topology-preserving only.",
      },
      {
        index: "03",
        meta: "`migrate` handles stateless apps and `movable_rwo` persistent storage; failover is for declared standby recovery.",
        title: "Move and failover are separate paths.",
      },
    ],
    notesIntro: "These are the edges users usually trip over on the first real deploy.",
  ...
  sections: [
    {
      ...
      label: "CLI",
      note: {
        body: "Normal text-mode commands can remind you that a newer GitHub Release exists. Use `fugue upgrade` to self-update, or set `FUGUE_SKIP_UPDATE_CHECK=1` when you need a quiet shell.",
      ...
      valueGroups: [
        {
          items: [
            ...
            {
              detail:
                "Set this on self-hosted control planes so CLI requests hit the right API origin.",
              label: "FUGUE_BASE_URL",
              value: "Self-hosted API base URL",
            },
            {
              detail:
                "Needed by admin snapshot and `fugue web diagnose` commands that talk to `fugue-web` routes.",
              label: "FUGUE_WEB_BASE_URL",
              value: "Self-hosted web base URL",
            },
            {
              detail:
                "Suppresses the release-upgrade reminder in the current shell session.",
    ...
    },
    {
      ...
      label: "fugue.yaml",
      note: {
        body: "Run `fugue template inspect github owner/repo` or `fugue deploy inspect github owner/repo` before importing a repository you did not author yourself. The inspect path shows `primary_service`, inferred warnings, template metadata, and any editable seed files.",
    ...
    },
    {
      ...
      codeExamples: [
        {
          caption: "These are the current high-value billing commands on the CLI side.",
      ...
      label: "Billing",
      note: {
        body: "The current price book is calibrated so a managed envelope of 2 vCPU, 4 GiB memory, and 30 GiB storage lands near four US dollars per month. That is a calibration target baked into the core billing model, not marketing copy.",
    ...
    },
    {
      ...
      label: "Continuity",
      note: {
        body: "The current managed failover implementation is strongest when the control plane coordinates both runtimes and their storage operators from the same Kubernetes control plane. Cross-cluster database bootstrap, promotion, and end-to-end recovery validation are not fully generic yet.",
...
const ZH_CN_CONTENT: DocsPageContent = {
  ...
  hero: {
    ...
    notes: [
      ...
      {
        index: "02",
        meta: "`persistent_storage_seed_files` 只有在拓扑识别完全保持自动模式时才可用。",
        title: "seed file 覆盖只能用于保拓扑导入。",
      },
      {
        index: "03",
        meta: "`migrate` 支持无状态应用和 `movable_rwo` 持久化存储；failover 用于已声明 standby 的恢复路径。",
  ...
  sections: [
    {
      ...
      label: "CLI",
      note: {
        body: "普通文本模式命令会在发现 GitHub Release 有更新时给出升级提醒。需要安静 shell 时，用 `FUGUE_SKIP_UPDATE_CHECK=1`；需要升级时，直接 `fugue upgrade`。",
        title: "升级提醒行为",
      },
      paragraphs: [
    ...
    },
    {
      ...
      tables: [
        ...
        },
        {
          ...
          rows: [
            ["Public", "创建 public route", "暴露 service port", "默认 HTTP 应用"],
            [
              "Internal",
              "不创建 public route",
    ...
    },
    {
      ...
      codeExamples: [
        {
          caption:
            "这个形状对应的是当前 importer 与 template inspector 真正理解的 `fugue.yaml`。",
          code: FUGUE_MANIFEST_SNIPPET,
          filename: "fugue.yaml",
          language: "yaml",
      ...
      label: "fugue.yaml",
      note: {
        body: "在导入自己没写过的仓库前，优先跑 `fugue template inspect github owner/repo` 或 `fugue deploy inspect github owner/repo`。它会先告诉你 `primary_service`、warning、template metadata 和可编辑 seed file，再决定是否真正导入。",
    ...
    },
    {
      ...
      label: "计费",
      note: {
        body: "当前默认 price book 的校准目标，是让 2 vCPU、4 GiB 内存、30 GiB 存储这组托管容量的大致月估值落在 4 美元附近。这是核心计费模型里的显式校准，不是 marketing 文案。",
    ...
    },
    {
      ...
      codeExamples: [
        {
          caption:
            "先 audit，再声明 failover target，最后在拓扑准备好后显式执行 failover。",
          code: FAILOVER_SNIPPET,
          filename: "continuity.sh",
          language: "bash",
      ...
      label: "连续性",
      note: {
        body: "当前 managed failover 最强的前提，是控制面能从同一个 Kubernetes control plane 协调两端 runtime 及其存储算子。跨集群数据库引导、提升与端到端恢复验证，还没有被抽象成完全通用的模型。",
        title: "当前边界",
      },
      paragraphs: [
