...
async function getBillingTopupRowForUpdate(client: PoolClient, requestId: string) {
...
export async function createBillingTopup(payload: {
  ...
  return toTopupRecord(requireQueryRow(result.rows[0], "Creating a billing top-up"));
}

export async function getBillingTopupForUser(userEmail: string, requestId: string) {
  await ensureDbSchema();
...
export async function claimBillingTopupForCompletion(
  ...
  return withDbTransaction(async (client) => {
    ...
    return {
      state: "claimed",
      topup: toTopupRecord(
        requireQueryRow(result.rows[0], "Claiming a billing top-up"),
      ),
    };
  });
}
