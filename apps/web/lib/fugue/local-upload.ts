...
function inferLocalUploadLabel(items: LocalUploadItem[], label?: string | null) {
...
export function normalizeLocalUploadItems(
  ...
  if (onlyItem && isSupportedLocalUploadArchivePath(onlyItem.path)) {
    return {
      kind: "archive",
      archive: {
        contentType: inferLocalUploadArchiveContentType(onlyItem.file, onlyItem.path),
        file: onlyItem.file,
        name: readBasename(onlyItem.path),
        size: onlyItem.size,
      },
      label:
        label?.trim() || stripArchiveExtension(readBasename(onlyItem.path)) || null,
    };
  }
...
export function inspectLocalUploadState(
  ...
  if (localUpload.kind === "archive") {
    ...
    const inspection: LocalUploadInspection = {
      archiveFormat: normalized.endsWith(".zip") ? "zip" : "tarball",
      archiveName,
      hasArchive: true,
      hasCompose: false,
      hasDockerfile: false,
...
export function hasLocalUploadSelection(
  localUpload: LocalUploadState | null | undefined,
) {
  if (!localUpload) {
    return false;
  }
