...
function buildCacheControlHeader(value: string | null) {
  ...
  for (const token of (value ?? "").split(",")) {
    ...
    const rawKey = trimmed.split("=", 1)[0] ?? trimmed;
...
function buildProxyHeaders(upstream: Response) {
  ...
  headers.set(
    "Cache-Control",
    buildCacheControlHeader(upstream.headers.get("Cache-Control")),
  );
  headers.set(
    "Content-Type",
...
export async function proxyFugueEventStream({
  ...
  if (!upstream.ok || !upstream.body) {
    const body = await readBoundedResponseBody(upstream).catch(() => "");
    const status = upstream.ok ? 502 : upstream.status;

    return new Response(sanitizePublicErrorMessage(body, status), {
      headers: {
