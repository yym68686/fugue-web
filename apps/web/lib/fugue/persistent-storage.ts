...
export function summarizePersistentStorageDraft(value: PersistentStorageMountDraft[]) {
  if (value.length === 0) {
    return null;
  }
...
export function validatePersistentStorageDraft(value: PersistentStorageMountDraft[]) {
...
export function serializePersistentStorageDraft(
  ...
  if (value.length === 0) {
    return options?.preserveEmpty
      ? ({ mounts: [] } satisfies PersistentStoragePayload)
      : undefined;
  }
...
export function readPersistentStorageInput(
  ...
  return {
    mounts: mountsValue.map((entry, index) => {
      if (!isRecord(entry)) {
        throw new Error(`Persistent storage mount ${index + 1} must be an object.`);
      }
      ...
      if (!path) {
        throw new Error(`Persistent storage mount ${index + 1} is missing a path.`);
      }
