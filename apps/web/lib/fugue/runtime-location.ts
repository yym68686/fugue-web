...
function readFirstRuntimeLabel(labels: RuntimeLabels, keys: readonly string[]) {
...
export function hasInternalClusterLocationTarget(labels: RuntimeLabels) {
  return Boolean(readFirstRuntimeLabel(labels, [INTERNAL_CLUSTER_LOCATION_KEY_LABEL]));
}

export function readRuntimeLocation(
  ...
  const locationCountryLabel = countryCode
    ? readCountryLabel(countryCode, locale)
    : null;
