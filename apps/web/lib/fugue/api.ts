...
import type { FugueAppObservabilityRequestOptions } from "@/lib/fugue/observability-request-query";
...
class FugueRequestError extends Error {
  readonly retryAfterSeconds: number | null;
  readonly status: number;

  constructor(message: string, status: number, retryAfterSeconds: number | null) {
    super(message);
    this.name = "FugueRequestError";
...
async function expectData<T>(
  ...
  const status =
    result.response.status >= 400 && result.response.status <= 599
      ...
      : 502;
  throw new FugueRequestError(
    sanitizePublicErrorMessage(readErrorDetail(result.error), status),
    status,
    readRetryAfterSeconds(result.response),
  );
}
...
async function expectMultipartData<T>(
  ...
  if (!response.ok) {
    ...
    throw new FugueRequestError(
      sanitizePublicErrorMessage(readErrorDetail(data), status),
      status,
  ...
  if (!data) {
    throw new FugueRequestError(sanitizePublicErrorMessage(null, 502), 502, null);
  }
...
function buildAuthContextView(principal: CamelizedSchema<"AuthPrincipalContext">) {
  return {
...
function buildTopologyInferenceView(inference: CamelizedSchema<"TopologyInference">) {
  return {
...
function buildProjectRouteDomainView(domain: CamelizedSchema<"ProjectRouteDomain">) {
  return {
    host: readNullableString(domain.host),
...
function buildProjectRouteBindingView(binding: CamelizedSchema<"ProjectRouteBinding">) {
  return {
...
function buildProjectRouteTableView(table: CamelizedSchema<"ProjectRouteTable">) {
  return {
...
function buildImportServiceDetailView(service: CamelizedSchema<"ImportServiceDetail">) {
  return {
    appId: service.appId,
...
function buildInspectGitHubTemplateManifestView(
  ...
  return {
    inferenceReport: (manifest.inferenceReport ?? []).map(buildTopologyInferenceView),
    domains: (manifest.domains ?? []).map(buildTopologyDomainView),
    entrypoints: (manifest.entrypoints ?? []).map(buildTopologyEntrypointView),
...
function buildInspectGitHubTemplateComposeStackView(
  ...
  return {
    ...
    services: (stack.services ?? []).map(buildInspectGitHubTemplateManifestServiceView),
...
function buildComposeStackSummaryView(
  ...
  return {
    ...
    inferenceReport: (summary.inferenceReport ?? []).map(buildTopologyInferenceView),
...
function buildInspectGitHubTemplateResponseView(
  ...
  return {
    composeStack: buildInspectGitHubTemplateComposeStackView(response.composeStack),
...
function buildAppPostgresView(postgres?: CamelizedSchema<"AppPostgresSpec"> | null) {
  return {
...
function buildAppSourceView(source?: CamelizedSchema<"AppSource"> | null) {
  return {
    ...
    buildContextDir: readNullableString(source?.buildContextDir),
    ...
    imageNameSuffix: readNullableString(source?.imageNameSuffix),
...
function buildAppView(app: CamelizedSchema<"App">) {
  ...
  // `source` is the contract's deprecated alias for `build_source`. Keep the
  // fallback only at this generated-contract adapter boundary until the
  // backend removes the alias after every supported deployment emits
  // build_source. Product consumers receive only the explicit buildSource.
  const buildSource = buildAppSourceView(app.buildSource ?? app.source);
  ...
  return {
    ...
    buildSource,
    originSource: buildAppSourceView(app.originSource ?? app.buildSource ?? app.source),
    spec: {
    ...
    status: {
      ...
      currentReleaseStartedAt: readNullableString(status?.currentReleaseStartedAt),
...
function buildDNSDelegationRecordView(record: CamelizedSchema<"DNSDelegationRecord">) {
  return {
...
function buildClusterNodeCpuStatsView(
  ...
  return {
    ...
    schedulableFreeMilliCores: readNullableNumber(stats.schedulableFreeMillicores),
...
function buildClusterNodePolicyView(
  ...
  return {
    ...
    effectiveControlPlaneRole: readNullableString(policy.effectiveControlPlaneRole),
...
function buildControlPlaneStatusView(status: CamelizedSchema<"ControlPlaneStatus">) {
  return {
...
function buildConsoleProjectBadgeView(badge: CamelizedSchema<"ConsoleProjectBadge">) {
  return {
...
function buildConsoleProjectSummaryView(
  ...
  return {
    ...
    serviceBadges: (summary.serviceBadges ?? []).map(buildConsoleProjectBadgeView),
...
function buildAppImageTrackingView(tracking: CamelizedSchema<"AppImageTracking">) {
  return {
...
function buildAppImageTrackingResultView(
  ...
  return {
    appId: response.appId,
    tracking: response.tracking ? buildAppImageTrackingView(response.tracking) : null,
  };
}
...
function buildAppPatchResultView(response: CamelizedSchema<"AppPatchResponse">) {
  return {
    alreadyCurrent: response.alreadyCurrent ?? false,
    app: response.app ? buildAppView(response.app) : null,
    operation: response.operation ? buildOperationView(response.operation) : null,
...
export type FugueAppListSort = Schemas["AppListSort"];
export type FugueCursorPageInfo = CamelizedSchema<"CursorPageInfo">;
export type FugueAppPage = {
  apps: FugueApp[];
  pageInfo: FugueCursorPageInfo;
};
...
export type FugueRuntimeDeleteResult = ReturnType<typeof buildDeleteRuntimeResultView>;
...
>;
export type FugueControlPlaneStatus = ReturnType<typeof buildControlPlaneStatusView>;
export type FugueRobustnessStatus = CamelizedSchema<"RobustnessStatus">;
export type FugueConsoleProjectBadge = ReturnType<typeof buildConsoleProjectBadgeView>;
...
>;
export type FugueConsoleGallery = ReturnType<typeof buildConsoleGalleryResponseView>;
...
>;
export type FugueHostedDNSZoneResult = ReturnType<typeof buildHostedDNSZoneResultView>;
...
>;
export type FugueAppImageTracking = ReturnType<typeof buildAppImageTrackingView>;
export type FugueAppImageTrackingResult = ReturnType<
  typeof buildAppImageTrackingResultView
>;
export type FugueAppImageSyncResult = ReturnType<typeof buildAppImageSyncResultView>;
...
>;
export type FugueProjectRouteTable = ReturnType<typeof buildProjectRouteTableView>;
  ...
  const response = camelizeData(
    await expectData(
      ...
      client.PATCH("/v1/projects/{id}", {
        body: {
          ...(payload.clearDefaultRuntimeId ? { clear_default_runtime_id: true } : {}),
...
export async function listFugueProjectRuntimeReservations(
  ...
  return (response.runtimeReservations ?? []).map(buildProjectRuntimeReservationView);
}
      ...
      client.POST("/v1/apps/import-github", {
        body: {
          ...
          ...(payload.persistentStorageSeedFiles?.length
            ? {
                persistent_storage_seed_files: payload.persistentStorageSeedFiles.map(
                  (file) => ({
                    path: file.path,
...
export async function importFugueUploadApp(
  ...
  body.set("archive", payload.archiveBody, payload.archiveName);
...
export async function getFugueAuthContext(accessToken: string) {
  const client = getClient(accessToken);
  const response = camelizeData(
    await expectData("/v1/auth/context", client.GET("/v1/auth/context")),
  );
...
export async function getFugueAppsPage(
  accessToken: string,
  options: {
    cursor?: string;
    domain?: string;
  ...
  if (!response.pageInfo) {
    throw new Error("Fugue returned an unpaginated app response.");
  }
    ...
    await expectData(
      ...
      client.PATCH("/v1/apps/{id}/source", {
        body: {
          origin_source: {
            type:
              payload.repoVisibility === "private" ? "github-private" : "github-public",
            repo_url: payload.repoUrl,
...
export async function getFugueAppImageTracking(accessToken: string, appId: string) {
  const client = getClient(accessToken);
  const response = camelizeData(
    await expectData(
...
export type FugueHostedDNSFlattenMode = NonNullable<
  components["schemas"]["HostedDNSRecord"]["flatten_mode"]
>;
export type FugueHostedDNSFlattenIPPolicy = NonNullable<
  components["schemas"]["HostedDNSRecord"]["flatten_ipv4_policy"]
>;
export type FugueHostedDNSFlattenTTLPolicy = NonNullable<
  components["schemas"]["HostedDNSRecord"]["flatten_ttl_policy"]
>;
export type FugueHostedDNSFlattenFallbackPolicy = NonNullable<
...
export async function deleteFugueHostedDNSZone(accessToken: string, zone: string) {
  const client = getClient(accessToken);
  const response = camelizeData(
    await expectData(
      `/v1/dns/zones/${encodeURIComponent(zone)}`,
...
export async function getFugueHostedDNSRecords(accessToken: string, zone: string) {
  const client = getClient(accessToken);
  const response = camelizeData(
    await expectData(
...
export async function listFugueAppObservabilityRequests(
  accessToken: string,
  appId: string,
  options?: FugueAppObservabilityRequestOptions,
) {
  const client = getClient(accessToken);
  const response = camelizeData(
    await expectData(
      ...
      client.GET("/v1/apps/{id}/observability/requests", {
        params: {
          ...
          query: {
            ...
            ...(options?.statusClass ? { status_class: options.statusClass } : {}),
            ...(typeof options?.statusCode === "number" &&
            Number.isFinite(options.statusCode)
              ? { status_code: options.statusCode }
              : {}),
            ...(typeof options?.slow === "boolean" ? { slow: options.slow } : {}),
            ...(typeof options?.errors === "boolean" ? { errors: options.errors } : {}),
          },
        },
      }),
    ),
...
export async function deleteFugueRuntime(accessToken: string, runtimeId: string) {
  const client = getClient(accessToken);
  const response = camelizeData(
    await expectData(
      ...
      client.PATCH("/v1/cluster/nodes/{name}/policy", {
        body: {
          ...(payload.allowBuilds !== undefined
            ...
            : {}),
          ...(payload.allowDns !== undefined ? { allow_dns: payload.allowDns } : {}),
          ...(payload.allowEdge !== undefined ? { allow_edge: payload.allowEdge } : {}),
...
export async function getFugueConsoleProject(accessToken: string, projectId: string) {
  const client = getClient(accessToken);
  const response = camelizeData(
    await expectData(
