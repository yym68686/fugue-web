...
import { buildRawEnvFeedback, type RawEnvFeedback } from "@/lib/console/raw-env";
...
export function localUploadPreservesDetectedTopology(draft: ImportServiceDraft) {
  return (
...
export function buildImportServicePayload(
  ...
  if (options?.includePersistentStorage !== false) {
    const persistentStorage = serializePersistentStorageDraft(draft.persistentStorage);
  ...
  if (draft.sourceMode === "github") {
    ...
    if (preservesGitHubTopologyImport(draft)) {
      const persistentStorageSeedFiles = draft.persistentStorageSeedFiles.flatMap(
        (file) => {
