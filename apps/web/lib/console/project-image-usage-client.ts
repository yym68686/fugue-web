...
import { createAbortRequestError, requestJson } from "@/lib/ui/request-json";
...
let projectImageUsageRequestCache: Promise<ProjectImageUsageSummary[]> | null = null;

export function readCachedProjectImageUsage() {
  if (
    !cachedProjectImageUsage ||
    Date.now() - cachedProjectImageUsage.cachedAt > PROJECT_IMAGE_USAGE_CACHE_TTL_MS
  ) {
...
export function buildProjectImageUsageMap(projects: ProjectImageUsageSummary[]) {
