...
function readSharedGroupStatus(
  ...
  const hasUniformStatus = targets.every(
    (target) => target.statusLabel === statusLabel && target.statusTone === statusTone,
  );
...
export function isManagedRuntimeTarget(target: ConsoleImportRuntimeTargetView) {
  return (
    target.runtimeType === "managed-owned" || target.runtimeType === "managed-shared"
  );
}
...
export function readManagedRuntimeTargets(
  ...
  return targets.filter(
    (target) => target.id !== excludedRuntimeId && isManagedRuntimeTarget(target),
  );
}
...
export function readDefaultImportRuntimeId(targets: ConsoleImportRuntimeTargetView[]) {
  return (
...
export function readSelectedRuntimeTargetGroupId(
  ...
  return (
    groups.find((group) => group.options.some((option) => option.id === runtimeId))
      ?.id ??
    groups[0]?.id ??
    null
  );
