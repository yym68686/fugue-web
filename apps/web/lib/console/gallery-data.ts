...
import { readGitHubBranchHref, readGitHubCommitHref } from "@/lib/fugue/source-links";
...
function readErrorMessage(reason: unknown) {
  return sanitizePublicErrorMessage(reason, readPublicErrorStatus(reason));
}

function isUnauthorizedFugueError(reason: unknown) {
  return readPublicErrorStatus(reason) === 401;
}
...
function buildWorkloadLocationMap(nodes: FugueClusterNode[], locale: Locale = "en") {
...
function buildRuntimeTargetLocationMap(
  ...
  for (const node of nodes) {
    ...
    for (const runtimeId of runtimeIds) {
      ...
      if (existing && !existing.locationCountryCode && !existing.locationLabel) {
...
function isManagedPostgresService(
  ...
  const normalizedProvisioner = service.provisioner?.trim().toLowerCase() ?? "";
...
function isDatabaseTransferOperation(operation?: FugueOperation | null) {
  ...
  return (
    ...
    (normalizedStatus.includes("database") && normalizedStatus.includes("switchover"))
  );
}
...
function databaseContinuitySnapshotsEqual(
  ...
  return (
    ...
    left.primaryPlacementPendingRebalance === right.primaryPlacementPendingRebalance &&
...
function isQueuedOperationStatus(operation?: FugueOperation | null) {
  const normalizedStatus = readNormalizedOperationStatus(operation);
  return normalizedStatus.includes("queued") || normalizedStatus.includes("pending");
}

function isDatabaseContinuityOperationCandidate(operation?: FugueOperation | null) {
  return (
...
function findBuildLogsOperationForCommit(
  ...
  if (options.preferActive) {
    return readActiveMatch(buildMatches) ?? buildMatches[0] ?? null;
  }

  return readTerminalMatch(buildMatches) ?? buildMatches[0] ?? null;
}
...
function readRunningBuildLogsOperation(
  ...
  const runningCommitSha = app.buildSource.commitSha?.trim() || null;
...
function readPendingBuildLogsOperation(
  ...
  return (
    releaseOperations.find(
      (operation) =>
        isBuildLogsOperationCandidate(operation) && isActiveOperation(operation.status),
    ) ??
    releaseOperations.find(
      (operation) =>
        isBuildLogsOperationCandidate(operation) &&
...
function readLatestFailedReleaseOperation(commitOperations?: AppCommitOperations) {
...
function hasSupersededReleaseFailure(commitOperations?: AppCommitOperations) {
  const supersedingSuccesses: FugueOperation[] = [];
...
function releaseOperationCanSupersedeFailure(
  ...
  return (
    isBuildLogsOperationCandidate(success) && isBuildLogsOperationCandidate(failure)
  );
}
...
function readActiveReleaseOperation(
  ...
  const runningCommit = app.buildSource.commitSha?.trim() || null;
  const activeRuntimeId = readActiveAppRuntimeId(app);
  const targetRuntimeId = operation.targetRuntimeId?.trim() || null;
...
function readRunningServiceMessage(
  ...
  if (activeOperation) {
    if (isTransferOperation(activeOperation)) {
      return readPendingCommitState(activeOperation).stateLabel === "Transfer queued"
...
function buildCommitViews(
  ...
  const runningCommit = buildCommitView({
    ...
    fallbackRepoUrl: app.buildSource.repoUrl,
    kind: "running",
    // `app.buildSource` stays on the live release until the deploy finishes.
...
function collectDatabaseTransferOperationsByAppId(sortedOperations: FugueOperation[]) {
...
function formatRouteLabel(hostname: string, pathPrefix?: string | null) {
  ...
  return normalizedPathPrefix === "/" ? hostname : `${hostname}${normalizedPathPrefix}`;
}
...
function readAppOriginSource(app: FugueApp) {
  return app.originSource ?? app.buildSource;
}
...
function readCurrentCommitLabel(app: FugueApp) {
  ...
  return shortCommitSha(app.buildSource.commitSha) || "Pending first import";
}
...
function readRedeployState(app: FugueApp) {
  ...
  return {
    canRedeploy: false,
    redeployDisabledReason: `Redeploy only works for imported GitHub, Docker image, or upload apps. Current source: ${humanize(app.buildSource.type)}.`,
  };
}
...
function buildBadgeFromTechStack(
  ...
  if (normalizedKind === "language" || normalizedKind === "stack") {
    const label =
      normalizedName || readTechnologyLabel(normalizedSlug) || humanize(normalizedSlug);
...
function buildAppBadges(
  ...
  const displaySource = options?.source ?? app.buildSource;
  ...
  const sourceLanguageBadge =
    sourceBadges.find((badge) => badge.meta === "Language" || badge.meta === "Stack") ??
    null;
...
function buildSharedAppView(
  ...
  const hasPublishedRoute = Boolean(routeHostname || app.route.publicUrl?.trim());
  ...
  return {
    ...
    exposesPublicRoute:
      hasPublishedRoute ||
      (app.spec.networkMode !== "background" && app.spec.networkMode !== "internal"),
    ...
    hasManagedPostgresService: app.backingServices.some((service) =>
      isManagedPostgresService(service),
    ),
    ...
    persistentStorageStorageSize: app.spec.persistentStorage?.storageSize ?? null,
  } satisfies Omit<
    ConsoleGalleryAppView,
...
function buildAppView(
  ...
  const primaryCommit =
    commitViews.find((entry) => entry.kind === "running") ?? commitViews[0] ?? null;
  ...
  const fallbackPhase = terminalFailureSuperseded
    ? "running"
...
function buildBackingServiceViews(
  ...
  const databaseFailoverTargetRuntimeId = postgres?.failoverTargetRuntimeId ?? null;
...
function buildImportRuntimeTargetView(
  ...
  const statusLabel = runtime.status
    ? translate(locale, humanize(runtime.status))
    : null;
...
function buildConsoleProjectSummaryView(
  ...
  const resourceUsageSnapshot = {
    ...
    ephemeralStorageBytes: project.resourceUsageSnapshot.ephemeralStorageBytes ?? null,
  ...
  return {
    ...
    lifecycle: {
      ...
      tone: readProjectLifecycleTone(project.lifecycle.label, project.lifecycle.tone),
    },
    name: project.name,
...
function buildConsoleProjectViewFromDetail(
  ...
  const appRuntimeIds = new Map(
    sortedApps.map(
      (app) =>
        [app.id, app.spec.runtimeId ?? app.status.currentRuntimeId ?? null] as const,
    ),
  );
  const appFailoverStates = new Map(
    sortedApps.map((app) => [app.id, readAppFailoverState(app)] as const),
  );
  const {
  ...
  const workloadLocationsById = buildWorkloadLocationMap(detail.clusterNodes, locale);
  ...
  const backingServiceViews = backingServices.flatMap((service) =>
    buildBackingServiceViews(
      ...
      service.ownerAppId
        ...
        : null,
      service.ownerAppId
        ? (databaseContinuityOperationsByAppId.get(service.ownerAppId) ?? null)
        : null,
    ).map((view) => ({
...
const getConsoleProjectGallerySummaryDataCached = cache(
  ...
  try {
    ...
    return {
      ...
      projects: gallery.projects.map((project) =>
        buildConsoleProjectSummaryView(project, locale),
      ),
      workspace: {
        exists: true,
...
async function loadRuntimeInventoryData(
  ...
  if (
    ...
    if (session) {
      const refreshed = await ensureWorkspaceAccess(session);
      [runtimesResult, clusterNodesResult] = await loadInventory(refreshed.workspace);
    }
  }

  const clusterNodes =
    clusterNodesResult.status === "fulfilled" ? clusterNodesResult.value : [];
  const runtimeTargetLocationsByRuntimeId = buildRuntimeTargetLocationMap(
    clusterNodes,
    locale,
  );
...
const getConsoleProjectGalleryDataCached = cache(
  async (
    ...
    const workloadLocationsById = buildWorkloadLocationMap(clusterNodes, locale);
    const runtimeTargetLocationsById = buildRuntimeTargetLocationMap(
      clusterNodes,
      locale,
    );
    ...
    const projectIds = [
      ...new Set([...projects.map((project) => project.id), ...appsByProjectId.keys()]),
    ];
...
export async function getConsoleProjectGallerySummaryData(locale: Locale = "en") {
...
export async function getConsoleProjectDetailDataForWorkspace(
  ...
  const project = buildConsoleProjectViewFromDetail(detail, locale);
  const firstService = project?.services[0];
  const firstApp = firstService?.kind === "app" ? firstService : null;
  const initialDomains = firstApp
    ? await requestWithWorkspaceRefresh(workspace, (active) =>
        getFugueAppDomains(active.adminKeySecret, firstApp.id),
      ).then(
        (data) => ({ appId: firstApp.id, data, error: null }),
        (error) => ({
          appId: firstApp.id,
          data: null,
...
export async function getConsoleRuntimeTargetInventoryData(locale: Locale = "en") {
  const workspace = await getCurrentWorkspaceAccess();
