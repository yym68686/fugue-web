...
const projectDetailRequestCache = new Map<string, Promise<ConsoleProjectDetailData>>();
...
export async function warmConsoleProjectDetails(
  ...
  const concurrency = Math.max(
    1,
    Math.min(options?.concurrency ?? PROJECT_DETAIL_PREFETCH_CONCURRENCY, queue.length),
  );
  let nextIndex = 0;
