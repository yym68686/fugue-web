...
function includesProjectLifecycleKeyword(value: string, keywords: readonly string[]) {
...
export function readProjectLifecycleTone(
  ...
  if (normalizedLabel) {
    ...
    if (
      includesProjectLifecycleKeyword(normalizedLabel, PROJECT_LIFECYCLE_INFO_KEYWORDS)
    ) {
      return "info";
    }
