...
function formatBytesLabel(
  ...
  return `${formatCompactNumber(amount, digits, locale)} ${units[unitIndex]}`;
}
...
function formatCPUMillicoresLabel(
  ...
  if (Math.abs(value) >= 1000) {
    ...
    const count = formatCompactNumber(cores, 1, locale);
    if (t) {
...
function formatVersionCountLabel(value: number, t?: ProjectResourceUsageTranslator) {
  if (t) {
...
export function buildProjectResourceUsageView(
  ...
  const imageTitleParts = hasImageUsage
    ? [
        `${t ? t("Image storage") : "Image storage"} / ${formatBytesLabel(imageTotalBytes, t, locale)} / ${
          t ? t("Stored project images") : "Stored project images"
        }`,
        imageUsage ? formatVersionCountLabel(imageUsage.versionCount, t) : null,
      ].filter((value): value is string => Boolean(value))
    : [];
  const cpuPrimaryLabel = formatCPUMillicoresLabel(usage.cpuMillicores, t, locale);
  const memoryPrimaryLabel = formatBytesLabel(usage.memoryBytes, t, locale);
  const diskPrimaryLabel = formatBytesLabel(usage.ephemeralStorageBytes, t, locale);
