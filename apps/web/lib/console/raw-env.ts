...
function sameEnvRecord(previous: Record<string, string>, next: Record<string, string>) {
...
export function areRawEnvFeedbackEqual(previous: RawEnvFeedback, next: RawEnvFeedback) {
  return (
...
export function buildRawEnvFeedback(
  ...
  return {
    env: parsed.env,
    message: buildSuccessMessage(envCount, surface, parsed.ignoredLineCount, locale),
    valid: true,
    variant: "success",
  };
}
