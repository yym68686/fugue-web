...
function stripInlineComment(value: string) {
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (index === 0 || /\s/.test(value.charAt(index - 1))) {
  ...
  while (true) {
    ...
    if (closingIndex >= 0) {
      ...
      if (trailing.length > 0 && !trailing.startsWith("#")) {
        return {
          message:
            "Unexpected characters after the quoted value. Use key=value or move comments onto their own line.",
          ok: false as const,
        };
      }
    ...
    collected += segment;
    collected += "\n";
    const nextSegment = lines[cursor];
...
function parseValue(rawValue: string, lines: string[], lineIndex: number) {
  ...
  const openingQuote = nextValue.charAt(0);

  if (openingQuote === '"' || openingQuote === "'") {
    return parseQuotedValue(nextValue, lines, lineIndex, openingQuote);
  }
...
export function parseRawEnvInput(input: string): EnvRawParseResult {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const parsedValue = parseValue(
      withoutExport.slice(separatorIndex + 1),
      lines,
      index,
    );
...
export function buildEnvDraftRowsFromEntries(
  ...
  for (const [key, value] of [...remaining.entries()].sort(([left], [right]) =>
