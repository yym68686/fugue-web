...
import type { FugueApp, FugueAppImageTracking, FugueAppSource } from "@/lib/fugue/api";
import {
...
function readSourceImageRef(app: FugueApp) {
  ...
  return source.imageRef?.trim() || app.buildSource.imageRef?.trim() || null;
}
...
function buildServiceView(
  ...
  const lastError = appTracking?.lastError ?? effectiveLink?.lastImageSyncError ?? null;
  const updatedAt =
...
export function buildProjectImageTrackingResponseView(input: {
  ...
  const linksByAppId = new Map(input.links.map((link) => [link.fugueAppId, link]));
  ...
  return {
    ...
    githubApp: buildGitHubAppView(input.binding, input.githubAppInstallation ?? null),
...
async function fetchGitHubWorkflowFiles(githubRepo: string, token?: string | null) {
  ...
  const workflowItems = contents.filter((item) => {
    ...
    return type === "file" && Boolean(path && /\.(ya?ml)$/i.test(path));
  });
...
function readWorkflowEnv(text: string, repo: GitHubRepoParts, defaultBranch: string) {
...
function resolveWorkflowExpression(
  ...
  for (let index = 0; index < 5; index += 1) {
    ...
    resolved = resolved.replace(GITHUB_EXPR_PATTERN, (_match, expression) => {
      ...
      if (envName) {
        const nextValue = env.get(envName.toUpperCase()) ?? "";
...
function readStepScalar(text: string, key: string) {
  const match = new RegExp(`^\\s*(?:-\\s*)?${key}\\s*:\\s*(.+?)\\s*$`, "m").exec(text);
...
function parseWorkflowFileCandidates(
  ...
  for (const step of steps) {
    ...
    const metadata =
      ...
      [...metadataSteps].reverse().find((candidate) => candidate.order < step.order) ??
      null;
    const images = Array.from(new Set([...(metadata?.images ?? []), ...directImages]));
