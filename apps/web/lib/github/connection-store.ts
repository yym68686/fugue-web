...
export type GitHubConnectionSnapshot = Omit<GitHubConnectionRecord, "accessToken">;
...
export async function getGitHubConnectionByEmail(email: string) {
  ...
  try {
    const { resealed, value } = resealTextIfNeeded(row.access_token_sealed);
    if (resealed) {
      await queryDb(
        `
          UPDATE app_github_connections
          SET access_token_sealed = $2, updated_at = NOW()
          WHERE user_email = $1
            AND access_token_sealed = $3
        `,
        [normalizeEmail(row.user_email), resealed, row.access_token_sealed],
      );
    }
    return recordFromRow(row, value);
  } catch {
    return null;
  }
}
