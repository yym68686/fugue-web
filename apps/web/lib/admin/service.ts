...
import {
  getAppBootstrapAdminEmail,
  listAppUsers,
  listAppUsersPage,
  type AppUserPageFilter,
  type AppUserRecord,
...
import {
  ...
  updateFugueBilling,
  type FugueApp,
  type FugueAppListSort,
  type FugueCursorPageInfo,
...
import { readCountryLocation, readLocalizedLocationLabel } from "@/lib/geo/country";
import {
  SUPPORTED_LOCALES,
...
export type AdminCursorPageInfo = {
  hasNextPage: boolean;
...
export type AdminAppsPageQuery = {
  cursor?: string;
  limit: number;
  phase?: string;
  query?: string;
  sort?: FugueAppListSort;
};
...
const adminUsersEnrichmentRequests = new Map<Locale, Promise<AdminUsersPageData>>();
const adminAppsPageRefreshRequests = new Map<Locale, Promise<AdminAppsPageData>>();
const adminAppsUsageRefreshRequests = new Map<Locale, Promise<AdminAppsUsageData>>();
const adminUsersPageRefreshRequests = new Map<Locale, Promise<AdminUsersPageData>>();
const adminUsersUsageRefreshRequests = new Map<Locale, Promise<AdminUsersUsageData>>();
const adminAppsPageDataCache = createExpiringAsyncCache<AdminAppsPageData>(
...
const adminAppsUsageDataCache = createExpiringAsyncCache<AdminAppsUsageData>(
  ADMIN_USAGE_CACHE_TTL_MS,
);
const adminUsersUsageDataCache = createExpiringAsyncCache<AdminUsersUsageData>(
  ADMIN_USAGE_CACHE_TTL_MS,
);
const adminUsersPageDataCache = createExpiringAsyncCache<AdminUsersPageData>(
  ADMIN_USAGE_CACHE_TTL_MS,
);
const adminControlPlaneViewCache = createExpiringAsyncCache<AdminControlPlaneView>(
...
function canRebuildApp(app: FugueApp) {
  const sourceType = app.buildSource.type?.trim().toLowerCase() ?? "";
...
function readEvidenceValue(evidence: Record<string, string> | undefined, key: string) {
  if (!evidence) {
    return null;
  }
...
function buildTrafficSafetyWarnings(
  ...
  return checks
    .filter((check) => check.name === "traffic_safety_min_healthy_edges" && !check.pass)
    .map((check, index): AdminTrafficSafetyWarningView => {
      ...
      return {
        ...
        message:
          check.message || "Healthy eligible edge nodes are below the service minimum.",
...
function buildAppStack(app: FugueApp) {
  ...
  if (app.techStack.length) {
    ...
    addItem(
      "language",
      app.buildSource.detectedProvider,
      app.buildSource.detectedProvider
        ? (readTechnologyLabel(app.buildSource.detectedProvider) ??
            humanize(app.buildSource.detectedProvider))
        : null,
      "fallback",
    );
    addItem(
      "build",
      app.buildSource.buildStrategy,
      app.buildSource.buildStrategy ? humanize(app.buildSource.buildStrategy) : null,
      "fallback",
    );
  ...
  const primary = items.filter(
    (item) =>
      item.kind === "stack" || item.kind === "language" || item.kind === "service",
  );
...
function mapAdminApps(
  ...
  const orderedApps = options?.preserveInputOrder
    ? apps
    : [...apps].sort(
        (left, right) =>
  ...
  return orderedApps.map((app) => {
    ...
    return {
      ...
      serverLabel: runtimeId
        ...
        : translate(locale, "Unassigned"),
      sourceHref: readFugueSourceHref(app.buildSource),
      sourceLabel: readFugueSourceLabel(app.buildSource),
...
async function getClusterProjects(bootstrapKey: string, tenants: FugueTenant[]) {
  try {
    return {
      errors: [],
...
function buildUserViews(
  ...
  let bootstrapAdminEmail: string | null = protectedAdminEmail ?? null;
  let bootstrapAdminCreatedAt = Number.POSITIVE_INFINITY;

  if (protectedAdminEmail === undefined) {
  ...
  const tenantServiceUsageByTenant = enrichment.loaded
    ? buildAdminTenantServiceUsageLookup(enrichment.apps, enrichment.appImageUsage)
  ...
  return users.map((user) => {
    ...
    return {
      billing: buildAdminUserBillingView(workspace, billing, enrichment.loaded, locale),
      canBlock: !user.isAdmin && user.status === "active",
      canDemoteAdmin:
        user.isAdmin && user.status !== "deleted" && user.email !== bootstrapAdminEmail,
      ...
      provider: readProviderLabel(user.provider, locale) ?? humanize(user.provider),
      serviceCount,
...
function buildAdminUsersSummary(users: AdminUserView[]): AdminUsersPageData["summary"] {
  return {
    adminCount: users.filter((user) => user.isAdmin).length,
    blockedCount: users.filter((user) => user.status.trim().toLowerCase() === "blocked")
      .length,
    deletedCount: users.filter((user) => user.status.trim().toLowerCase() === "deleted")
      .length,
...
function formatCompactNumber(value: number, digits = 1, locale: Locale = "en") {
...
function formatBillingResourceSpec(spec: FugueResourceSpec, locale: Locale = "en") {
  const parts = [
...
function buildAdminUserBillingView(
  ...
  if (!billingLookup?.billing) {
    return {
      ...
      loadError:
        billingLookup?.error ?? "Fugue billing is unavailable for this workspace.",
      loading: false,
      memoryMebibytes: null,
      monthlyEstimateLabel: null,
      priceBook: null,
...
function buildAdminAppResourceUsage(
  ...
  const diskPrimaryLabel = formatBytesLabel(usage?.ephemeralStorageBytes, locale);
...
function buildCPUResourceView(
  ...
  const requestedLabel = formatCPUCapacityLabel(stats?.requestedMilliCores, locale);
  ...
  return {
    ...
    requestAmountLabel: isFiniteResourceValue(requestPercent) ? requestedLabel : null,
    ...
    schedulableFreeAmountLabel: isFiniteResourceValue(stats?.schedulableFreeMilliCores)
...
function buildMemoryResourceView(
  ...
  const capacityAmountLabel = isFiniteResourceValue(capacityBytes)
    ...
    : null;
  const schedulableFreeLabel = formatBytesLabel(stats?.schedulableFreeBytes, locale);
  const statusPercent = maxFinitePercent(percent, requestPercent);
  ...
  return {
    ...
    requestPercentValue: requestPercent,
    requestTone: readResourceTone(requestPercent),
    schedulableFreeAmountLabel: isFiniteResourceValue(stats?.schedulableFreeBytes)
      ? schedulableFreeLabel
      : null,
    schedulableFreeLabel: buildSchedulableFreeLabel(
...
function buildClusterWorkloadViews(
  ...
  return workloads.map((workload) => {
    ...
    const podLabel = workload.pods.length
      ? workload.pods
          .map((pod) =>
            pod.phase?.trim() ? `${pod.name} (${humanize(pod.phase)})` : pod.name,
          )
          .join(" / ")
...
function buildClusterNodeViewItem(
  ...
  const pressureSignals = conditionViews.filter(
    (condition) =>
      condition.id !== CLUSTER_READY_CONDITION && condition.statusLabel === "Pressure",
  );
...
function buildAdminCursorPageInfo(pageInfo: FugueCursorPageInfo): AdminCursorPageInfo {
  return {
...
export async function getAdminAppsBoundedPageData(
  options: AdminAppsPageQuery,
  locale: Locale = "en",
): Promise<AdminAppsPageData> {
  const bootstrapKey = getFugueEnv().bootstrapKey;
  const page = await getFugueAppsPage(bootstrapKey, {
  ...
  const workspacesResult = await settleResult(
    getWorkspaceSnapshotsByTenantIds(
      page.apps
        .map((app) => app.tenantId)
  ...
  const latestUpdateAt = page.apps.reduce<string | null>((latest, app) => {
  ...
  return {
    apps: views,
    errors,
    pageInfo: buildAdminCursorPageInfo(page.pageInfo),
    summary: {
      appCount: page.pageInfo.totalItems,
      ...
      tenantCount: new Set(page.apps.map((app) => app.tenantId).filter(Boolean)).size,
    },
  };
}
...
async function loadAdminAppsPageData(
  ...
  const views = applyAdminAppsUsageData(
    mapAdminApps(apps, projects, workspaces, tenants, appImageUsage, runtimes, locale),
    usageData,
  );
...
async function readPersistedAdminAppsPageData(locale: Locale = "en") {
  const cacheKey = readLocalizedAdminCacheKey(ADMIN_APPS_PAGE_DATA_CACHE_KEY, locale);
  const entry = await readAdminSnapshotCache<AdminAppsPageData>(cacheKey).catch(
    () => null,
  );
...
async function readPersistedAdminAppsUsageData(locale: Locale = "en") {
  const cacheKey = readLocalizedAdminCacheKey(ADMIN_APPS_USAGE_DATA_CACHE_KEY, locale);
  const entry = await readAdminSnapshotCache<AdminAppsUsageData>(cacheKey).catch(
    () => null,
  );
...
export async function getAdminAppsUsageDataFast(
  ...
  const result = await settleWithSoftTimeout(refreshAdminAppsUsageData(locale), waitMs);
...
async function readPersistedAdminUsersUsageData(locale: Locale = "en") {
  const cacheKey = readLocalizedAdminCacheKey(ADMIN_USERS_USAGE_DATA_CACHE_KEY, locale);
  const entry = await readAdminSnapshotCache<AdminUsersUsageData>(cacheKey).catch(
    () => null,
  );
...
async function getAdminUserBillingSummaryWithRetry(
  ...
  for (let attempt = 0; attempt <= ADMIN_BILLING_LOOKUP_RETRY_COUNT; attempt += 1) {
    try {
...
export async function getAdminUsersBoundedPageData(
  options: AdminUsersPageQuery,
  locale: Locale = "en",
  ...
  const [workspacesResult, bootstrapAdminResult] = await Promise.all([
    settleResult(getWorkspaceSnapshotsByEmails(page.users.map((user) => user.email))),
    settleResult(getAppBootstrapAdminEmail()),
  ]);
  const workspaces =
    workspacesResult.status === "fulfilled" ? workspacesResult.value : [];
  const errors = [
    workspacesResult.status === "rejected"
      ? `workspaces: ${readErrorMessage(workspacesResult.reason)}`
      : null,
    bootstrapAdminResult.status === "rejected"
      ? `bootstrap admin: ${readErrorMessage(bootstrapAdminResult.reason)}`
      : null,
  ...
  const data = buildAdminUsersPageData(
    ...
    "ready",
    locale,
    bootstrapAdminResult.status === "fulfilled"
      ? bootstrapAdminResult.value
      : undefined,
  );
  ...
  return {
    ...
    users: data.users.map((user) => ({
      ...user,
      usage: {
        cpuLabel: translate(locale, "Unavailable"),
        diskLabel: translate(locale, "Unavailable"),
        imageLabel: translate(locale, "Unavailable"),
        loading: false,
        memoryLabel: translate(locale, "Unavailable"),
        serviceCount: 0,
        serviceCountLabel: translate(locale, "Unavailable"),
      },
    })),
  };
}
...
async function readPersistedAdminUsersPageData(locale: Locale = "en") {
  const cacheKey = readLocalizedAdminCacheKey(ADMIN_USERS_PAGE_DATA_CACHE_KEY, locale);
  const entry = await readAdminSnapshotCache<AdminUsersPageData>(cacheKey).catch(
    () => null,
  );
...
export async function getAdminControlPlaneData(locale: Locale = "en"): Promise<{
  controlPlane: AdminControlPlaneView | null;
  errors: string[];
}> {
  let bootstrapKey: string;
...
export async function getAdminClusterPageData(
  ...
  const [nodesResult, tenantsResult, controlPlaneResult, robustnessResult] =
    await Promise.all([
      settleResult(
...
export async function setAdminClusterNodePolicy(
  ...
  const bootstrapKey = getFugueEnv().bootstrapKey;
  const result = await setFugueClusterNodePolicy(bootstrapKey, nodeName, payload);
  const tenants = await getFugueTenants(bootstrapKey).catch(() => [] as FugueTenant[]);
...
export async function createAdminPlatformNodeEnrollment(payload?: { label?: string }) {
