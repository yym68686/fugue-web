...
const ANSI_OSC_PATTERN = new RegExp(
  `${ANSI_ESCAPE}\\][^${ANSI_BELL}]*(?:${ANSI_BELL}|${ANSI_ESCAPE}\\\\)`,
  "g",
);
...
function stripUnsupportedAnsi(value: string) {
  return value.replace(ANSI_OSC_PATTERN, "").replace(ANSI_CONTROL_PATTERN, "");
}
...
function readNamedTone(value: number): AnsiNamedTone | null {
  if (value >= 0 && value <= 7) {
    return ANSI_BASE_TONES[value] ?? null;
  }

  if (value >= 8 && value <= 15) {
    const baseTone = ANSI_BASE_TONES[value - 8];
    return baseTone ? (`bright-${baseTone}` as AnsiNamedTone) : null;
  }
...
function readAnsi256Color(value: number) {
  ...
  if (normalized >= 16 && normalized <= 231) {
    ...
    const channel = [0, 95, 135, 175, 215, 255] as const;
...
function applyForeground(state: AnsiState, code: number) {
  if (code >= 30 && code <= 37) {
    state.tone = readNamedTone(code - 30);
    state.color = null;
    return;
  }

  if (code >= 90 && code <= 97) {
    state.tone = readNamedTone(code - 90 + 8);
    state.color = null;
  }
}
  ...
  for (let index = 0; index < codes.length; index += 1) {
    ...
    switch (code) {
      ...
      case 38: {
        ...
        const indexedColor = codes[index + 2];

        if (mode === 5 && indexedColor !== undefined) {
          const tone = readNamedTone(indexedColor);

          state.tone = tone;
          state.color = tone ? null : readAnsi256Color(indexedColor);
          index += 2;
        } else if (mode === 2) {
          const rawRed = codes[index + 2];
          const rawGreen = codes[index + 3];
          const rawBlue = codes[index + 4];

          if (rawRed === undefined || rawGreen === undefined || rawBlue === undefined) {
            break;
          }

          const red = clampByte(rawRed);
          const green = clampByte(rawGreen);
          const blue = clampByte(rawBlue);
...
export function parseAnsiText(value: string) {
  ...
  for (
    let match = ANSI_SGR_PATTERN.exec(source);
    match;
    match = ANSI_SGR_PATTERN.exec(source)
  ) {
