...
import type { ApiKeyRecord, ApiKeySource, ApiKeyStatus } from "@/lib/api-keys/types";
...
import { resealTextIfNeeded, sealText } from "@/lib/security/seal";
...
async function getApiKeyRow(client: PoolClient, email: string, fugueKeyId: string) {
...
async function upsertVisibleApiKey(client: PoolClient, input: SaveApiKeyInput) {
  ...
  const disabledAt =
    status === "disabled" ? (readTimestamp(input.apiKey.disabledAt) ?? now) : null;
  ...
  return withDbTransaction(async (client) => {
    ...
    for (const apiKey of input.visibleKeys) {
      await upsertVisibleApiKey(client, {
        ...
        email: input.email,
        source:
          apiKey.id === input.workspaceAdminKeyId ? "workspace-admin" : "external",
    ...
    await client.query(
      ...
      `,
      [normalizeEmail(input.email), input.tenantId, now, visibleKeyIds],
    );
...
export async function getApiKeySecret(email: string, fugueKeyId: string) {
  ...
  const { resealed, value } = resealTextIfNeeded(secretSealed);
  if (resealed) {
    await queryDb(
      `
        UPDATE app_api_keys
        SET secret_sealed = $3, updated_at = NOW()
        WHERE user_email = $1
          AND fugue_key_id = $2
          AND secret_sealed = $4
      `,
      [normalizeEmail(email), fugueKeyId, resealed, secretSealed],
    );
  }
  return value;
}
