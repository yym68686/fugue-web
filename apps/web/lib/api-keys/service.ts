...
import { getWorkspaceAccessByEmail, saveWorkspaceAccess } from "@/lib/workspace/store";
...
function readAllowedScopesForKey(
  workspaceAdminScopes: string[],
...
function readMutationAccessToken(
  isWorkspaceAdmin: boolean,
  workspaceAdminSecret: string,
) {
  if (isWorkspaceAdmin) {
...
async function persistApiKeyMutation(input: {
  ...
  if (isWorkspaceAdmin) {
    const shouldUseAsWorkspaceAdmin =
      Boolean(secret) || apiKey.id === workspace.adminKeyId;
...
export async function getApiKeyPageDataForWorkspace(
  ...
  try {
    ...
    const currentAdminKey = visibleKeys.find(
      (key) => key.id === currentWorkspace.adminKeyId,
    );

    if (currentAdminKey) {
      const desiredScopes = readDesiredWorkspaceAdminScopes(
        currentWorkspace,
        currentAdminKey,
      );

      if (!sameStringArray(sortFugueScopes(currentAdminKey.scopes), desiredScopes)) {
        const updatedAdminKey = await updateFugueApiKey(
          getFugueEnv().bootstrapKey,
          currentAdminKey.id,
          {
