...
import { getAdminClusterPageData, getAdminControlPlaneData } from "@/lib/admin/service";
...
function readIncludeControlPlane(request: Request) {
  return new URL(request.url).searchParams.get("include_control_plane") === "1";
}

export async function GET(request: Request) {
  const access = await requireAdminSnapshotApiSession("admin.snapshot.cluster.read");
