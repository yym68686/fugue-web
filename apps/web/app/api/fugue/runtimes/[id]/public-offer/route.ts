...
function readOptionalPayloadNumber(
  ...
  return (
    readOptionalNumber(body[snakeCaseKey]) ?? readOptionalNumber(body[camelCaseKey])
  );
}
...
function readOptionalPayloadBoolean(
  ...
  return (
    readOptionalBoolean(body[snakeCaseKey]) ?? readOptionalBoolean(body[camelCaseKey])
  );
}
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const sharing = await setRuntimePublicOfferForEmail(session.email, runtimeId, {
      free: readOptionalBoolean(body.free),
      freeCpu: readOptionalPayloadBoolean(body, "free_cpu", "freeCpu"),
      freeMemory: readOptionalPayloadBoolean(body, "free_memory", "freeMemory"),
      freeStorage: readOptionalPayloadBoolean(body, "free_storage", "freeStorage"),
