...
import { importFugueDockerImageApp, importFugueGitHubApp } from "@/lib/fugue/api";
import {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const resolvedAppName =
      sourceMode === "github"
        ? resolveDeployAppName([name, readRepositoryAppNameCandidate(repoUrl)], {
            fallbackSeeds: [repoUrl],
          })
        : resolveDeployAppName([name, readImageAppNameCandidate(imageRef)], {
    ...
    const resolvedProjectId =
      resolvedExistingProject?.id ?? result.app?.projectId ?? null;
