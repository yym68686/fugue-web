...
import { enableApiKeyForEmail } from "@/lib/api-keys/service";
...
function readErrorStatus(error: unknown) {
  ...
  if (error.message.includes("cannot be enabled")) {
    return 400;
  }
...
export async function POST(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    const enabled = await enableApiKeyForEmail(session.email, await readKeyId(context));
