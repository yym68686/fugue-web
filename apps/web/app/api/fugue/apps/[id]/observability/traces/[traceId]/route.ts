...
import { getFugueAppObservabilityTrace } from "@/lib/fugue/api";
import {
  jsonError,
  readErrorMessage,
  readErrorStatus,
...
type RouteContext = RouteContextWithParams<"id" | "traceId">;
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    const traceId = await readRouteParam(context, "traceId");
    const result = await getFugueAppObservabilityTrace(
