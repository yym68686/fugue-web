...
function readPersistentStorageSeedFiles(value: unknown) {
  ...
  return value.map((entry, index) => {
    if (!isObject(entry)) {
      throw new Error(`Persistent storage file ${index + 1} must be an object.`);
    }

    const service = typeof entry.service === "string" ? entry.service.trim() : "";
    const path = typeof entry.path === "string" ? entry.path.trim() : "";
    const seedContent = typeof entry.seedContent === "string" ? entry.seedContent : "";

    if (!service) {
      throw new Error(`Persistent storage file ${index + 1} is missing a service.`);
    }

    if (!path) {
      throw new Error(`Persistent storage file ${index + 1} is missing a path.`);
    }

    if (entry.seedContent !== undefined && typeof entry.seedContent !== "string") {
      throw new Error(`Persistent storage file ${index + 1} must use text content.`);
    }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (
      templateSlug &&
      inspection.template?.slug &&
      inspection.template.slug !== templateSlug
    ) {
      return jsonError(
        409,
        "Template metadata changed. Reload the page and try again.",
      );
    }
    ...
    const resolvedAppName = resolveDeployAppName(
      [name, inspection.repository.defaultAppName, inspection.repository.repoName],
      {
    ...
    const resolvedProjectName = resolvedExistingProject?.name ?? requestedProjectName;
    ...
    const templateEnv = resolveTemplateEnv(inspection, readStringMap(body.variables));
    ...
    const result = await importFugueGitHubApp(workspace.adminKeySecret, {
      ...
      persistentStorageSeedFiles:
        persistentStorageSeedFiles.length > 0 ? persistentStorageSeedFiles : undefined,
      repoAuthToken: repoAccess.token || undefined,
      repoUrl,
      repoVisibility: resolvedRepoVisibility,
