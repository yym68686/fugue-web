...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    const nodeName = await readRouteParam(context, "name");
    const payload = {
      ...
      desiredControlPlaneRole: Object.hasOwn(body, "desiredControlPlaneRole")
        ? readOptionalString(body, "desiredControlPlaneRole")
        : undefined,
    };
    const node = await runAuditedOutboundAdminMutation(
      {
        actorEmail: access.session.email,
        changesAllowBuilds: payload.allowBuilds !== undefined,
        changesAllowDns: payload.allowDns !== undefined,
        changesAllowEdge: payload.allowEdge !== undefined,
        changesAllowSharedPool: payload.allowSharedPool !== undefined,
        changesControlPlaneRole: payload.desiredControlPlaneRole !== undefined,
        kind: "cluster.node-policy",
        nodeName,
      },
      () => setAdminClusterNodePolicy(nodeName, payload),
    );
