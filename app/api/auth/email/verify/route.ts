...
export async function GET(request: Request) {
  ...
  if (!token) {
    return NextResponse.redirect(buildAppUrl("/auth/sign-in?error=invalid-token"), {
      status: 303,
    });
  }
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes("blocked")) {
      return NextResponse.redirect(buildAppUrl("/auth/sign-in?error=account-blocked"), {
        status: 303,
      });
    }

    if (error instanceof Error && error.message.includes("deleted")) {
      return NextResponse.redirect(buildAppUrl("/auth/sign-in?error=account-deleted"), {
        status: 303,
      });
    }
  ...
  const destination = buildAppUrl("/app");
